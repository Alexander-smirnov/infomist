...
foreach($cfg['category'] as $post_type => $taxonomies)
{
	...
	if (have_posts())
	{
		...
		$hide =  get_post_meta($post->ID, 'news_yandex_ru_hide', true);
			...
			<item>
				...
				// added
				$taxonomy = reset(array_keys($cfg['category'][get_post_type($post->ID)]));
				$categories = get_the_terms($post->ID, $taxonomy);
				$category = reset($categories);
				 echo "<category>".news_yandex_ru_formatchar(htmlspecialchars(strip_tags($category->name, ENT_QUOTES)))."</category>\n";
				 // commented
