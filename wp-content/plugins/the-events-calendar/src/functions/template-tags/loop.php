...
if ( class_exists( 'Tribe__Events__Main' ) ) {
	...
	function tribe_get_events_title( $depth = true ) {
		...
		$title = sprintf( __( 'Найближчі %s', 'tribe-events-calendar' ), $events_label_plural );
		...
		if ( isset( $_REQUEST['tribe-bar-date'] ) && $wp_query->have_posts() ) {
			...
			$title = sprintf( __( '%1$s на %2$s - %3$s', 'tribe-events-calendar'), $events_label_plural, $first_event_date, $last_event_date );
		...
		if ( tribe_is_day() ) {
			$title = sprintf( 
				__( '%1$s на %2$s', 'tribe-events-calendar' ),
				$events_label_plural,
